...
class ABFS_match:
    ...
    def __init__(self, n_features,
             ...
             reset_on_drift=False,       # ablation switch
             weight_propagation=False,   # ablation switch
             seed=42):
        """
        Parameters:
    ...
    def update(self, x_arr, y):
        ...
        for j in range(self.n_features):
            ...
            if self.weight_propagation:
                # ORIGINAL-ABFS online boosting (Oza) — VERIFY the lambda rule
                # against Barddal et al. before trusting the weight-prop runs.
                ...
                eps = self._lam_wrong[j] / total if total > 0 else 0.5
                eps = min(max(eps, 1e-6), 1 - 1e-6)
                lam *= (1.0 / (2.0 * (1.0 - eps))) if correct else (1.0 / (2.0 * eps))
            else:
                self.stumps[j].learn_one(x_single, y)

...
measures = ["model-based"]

# -------------------- NEW -------------------------

# Replace the out = np.zeros(...) line with dynamic sizing
# Run one chunk first to get the actual size, then allocate

# Before the main loop, get actual feature counts
actual_measures_len = []
stream_test = sl.streams.StreamGenerator(**{**stream_static, **drift_types['sudden'], 'random_state': 42})
X_test, y_test = stream_test.get_chunk()
for measure_key in measures:
    mfe = MFE(groups=[measure_key])
    mfe.fit(X_test, y_test)
    ft_labels, ft = mfe.extract()
    actual_measures_len.append(len(ft))
    print(f"{measure_key}: {len(ft)} features")
...
for m_id, measure_key in enumerate(measures):
    ...
    out = np.zeros((len(drift_types), len(random_states), stream_static['n_chunks'], actual_measures_len[m_id]+1))
    pbar.reset()
    print(measure_key)
    ...
    np.save(os.path.join(RESULTS_DIR, '%s.npy' % measure_key), out)

...
# chunk_size=200.
#
# IMPORTANT — what "concept" means here:
#   Table 2 only tells us WHERE a change point occurs, not WHAT the
#   stream changes into. I do not know the nature of the change at each
#   point — only that one occurred. The concept label I assign is purely
#   positional: concept = number of change points passed so far. So the
#   segment before the first change point is concept 0, the segment
#   between the first and second is concept 1, and so on. This means:
#     - each segment is treated as a distinct concept, even if two
#       segments happen to be statistically similar to each other
#       (no recurring-concept structure is assumed or detected)
#     - "concept classification" here means "which segment of the
#       stream (between known boundaries) does this window belong to",
#       not "which semantically distinct regime is this"
#   This is the same construction used for poker-lsn
#   (concept = count of artificial sorting boundaries passed).
#
...
#       last column = integer concept label (0-indexed, positional)
...
# INSECTS species ID -> 0-indexed encoding (used for the raw species
# label; NOT the concept label, which is positional — see header note)
...
for stream_name, (csv_fname, change_points) in INSECTS_STREAMS.items():
    ...
    for idx, line in enumerate(load_csv(csv_path)):
        ...
        # the concept label is positional (see header note) — assigned
        # after loading, based on change_points, not the species ID
    ...
    # positional concept label: count of change points passed by this
    # INSTANCE index (not chunk index — finer-grained, then chunked
    # the same way as the rest of the pipeline via chunk-level voting
    # downstream in evaluate_concept_classification_3.py)
    y_concept = np.array([
        int(np.sum(np.array(change_points) <= i)) for i in range(n_instances)
    ])

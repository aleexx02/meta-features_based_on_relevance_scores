...
# #   --shap        : SHAP feature importance per cell
...
# #   HT  — DecisionTreeClassifier (proxy for Hoeffding Tree)
# #   MLP — MLPClassifier
...
#                                         f'metafeatures_{version}_{tag}.png')
#                     if not os.path.exists(fname):
#                         n_cols = 5
...
#             for n_informative in N_INFORMATIVES:
#                 tag   = make_tag(chunk_size, n_informative, drift_type)
#                 # skip if all 4 clf figures exist for this tag
#                 all_done = all(
...
#                         ax.bar(range(len()), mean_abs_shap[sorted_idx],
...
#                                  f'({N_REPLICATIONS} replications combined)', fontsize=12)
#                     plt.tight_layout()

...
RESULTS_DIR = os.path.join(PROJECT_ROOT, 'results/experiment_0') # .npy files go here
...
for drift_idx, drift_type, n_concepts in DRIFT_TYPES:
    ...
    for ax, matrix, title in [
        ...
        (axes[1], rc_matrix_ba_plot, f'Replication check  - {drift_type} drift'),
        (axes[2], diff, f'Difference: Our protocol - E2 ({drift_type} drift)')
    ]:
    ...
    fig.suptitle(f'Replication check - {drift_type} drift - Balanced Accuracy', fontsize=13)
    plt.tight_layout()

# analysis_3.py
# ============================================================
# Analysis of Experiment 3 results (INSECTS real-world streams).
#
# Usage:
#   python experiments/experiment_3/analysis_3.py --sanity --performance --shap --metrics
#
# Flags:
#   --sanity      : relevance scores over time, meta-features per window, PCA
#   --performance : cumulative BA trajectory over windows per stream
#   --shap        : SHAP feature importance per stream per ABFS version
#   --metrics     : F1 and Kappa heatmaps per stream
#
# Key differences from analysis_2.py:
#   - No replications: single fixed stream per name
#   - No grid: 3 streams only
#   - No variance plots: prequential only, no CV folds
#   - Drift boundaries from ground truth files, not generated
#   - 9 Komorniczak measure groups + 3 ABFS versions
#
...
#  ARGUMENT PARSING
# ============================================================
parser = argparse.ArgumentParser(description='Analysis for Experiment 3.')
parser.add_argument('--sanity',      action='store_true',
                    help='Relevance scores, meta-features per window, PCA')
parser.add_argument('--performance', action='store_true',
                    help='Cumulative BA trajectory plots')
parser.add_argument('--shap',        action='store_true',
                    help='SHAP feature importance plots')
parser.add_argument('--metrics',     action='store_true',
                    help='F1 and Kappa heatmaps')
...
print(f"Performance trajectories : {RUN_PERFORMANCE}")
print(f"SHAP                     : {RUN_SHAP}")
print(f"Metrics (F1, Kappa)      : {RUN_METRICS}")
...
MF_NAMES_RAW = [f'r_f{j+1}' for j in range(N_FEATURES)]
...
def load_gt(stream_name):
    return np.load(os.path.join(INSECTS_GT_DIR, f'{stream_name}.npy'))


def get_drift_boundaries_in_windows(drift_chunks):
    return list(drift_chunks)


def extract_stream_features(stream_name, drift_chunks):
    """
    Single pass over a stream: extract relevance scores over time
    and raw v2.0 meta-features per window.

    Returns
    -------
    scores_over_time : np.ndarray (n_windows, N_FEATURES)
    X_raw            : np.ndarray (n_windows, N_FEATURES)
    ...
    scores_over_time, mf_raw_list, concept_labels = [], [], []
    wt_prev = None
    ...
    for chunk_idx in range(100000):
        ...
        mf_raw_list.append(extract_metafeatures_raw(wt))
        concept_labels.append(concept)
        wt_prev = wt

    scores = np.array(scores_over_time)
    X_raw  = np.array(mf_raw_list, dtype=float)
    y      = np.array(concept_labels)
    X_raw[np.isnan(X_raw)] = 0
    X_raw[np.isinf(X_raw)] = 0
    return scores, X_raw, y


# ============================================================
#  0. SANITY CHECK PLOTS
# ============================================================
if RUN_SANITY:
    print("\n" + "="*60)
    ...
    for stream_name in INSECTS_STREAMS:
        ...
        scores_over_time, X_raw, y = extract_stream_features(stream_name, drift_chunks)
        ...
        if not os.path.exists(fname):
            ...
            x_axis = np.arange(len(scores_over_time))
            for j in range(N_FEATURES):
                ax.plot(x_axis, scores_over_time[:, j],
                        label=f'f{j+1}', linewidth=0.7, alpha=0.8)
            for b in boundaries:
            ...
            ax.set_title(f'Relevance scores over time — {stream_name}\n'
        ...
        fname = os.path.join(FIGURES_DIR, f'metafeatures_{stream_name}.png')
        if not os.path.exists(fname):
            n_cols = 5
            ...
            for k in range(N_FEATURES):
                axes_flat[k].plot(X_raw[:, k], color='steelblue', linewidth=0.8)
                for b in boundaries:
                    axes_flat[k].axvline(x=b, color='red', linestyle='--',
                                         linewidth=0.8, alpha=0.7)
                axes_flat[k].set_title(f'r_f{k+1}', fontsize=8)
                axes_flat[k].set_xlabel('Window', fontsize=7)
            for k in range(N_FEATURES, len(axes_flat)):
                axes_flat[k].set_visible(False)
            fig.suptitle(f'Meta-features (raw v2.0) per window — {stream_name}',
                         fontsize=10)
            fig.tight_layout()
            fig.savefig(fname, dpi=150, bbox_inches='tight')
            plt.close()
        ...
        # ---- PCA ----
        fname = os.path.join(FIGURES_DIR, f'pca_{stream_name}.png')
        if not os.path.exists(fname):
            colors = {c: PALETTE[i % len(PALETTE)]
                      for i, c in enumerate(unique_concepts)}
            pca = PCA(n_components=2)
            projected = pca.fit_transform(X_raw)
            ...
            ax.set_title(f'PCA of meta-feature vectors (raw v2.0)\n{stream_name}')
...
if RUN_PERFORMANCE:
    ...
    for stream_name in INSECTS_STREAMS:
        ...
        random_baseline = 1.0 / n_concepts
        drift_chunks    = load_gt(stream_name)
        boundaries      = get_drift_boundaries_in_windows(drift_chunks)

        print(f"\n  {stream_name}")

        # ABFS: all 3 versions stacked on one figure
        ...
        if not os.path.exists(fname):
            ...
            fig.suptitle(
                f'Exp 3 — ABFS performance trajectories\n{stream_name} '
                f'({n_concepts} concepts | random baseline={random_baseline:.3f})',
                fontsize=12)
            plt.tight_layout()
        ...
        # Komorniczak: all 9 measure groups in a 3x3 subplot grid
        ...
        if not os.path.exists(fname):
            ...
            fig.suptitle(
                f'Exp 3 — Komorniczak performance trajectories\n{stream_name}',
                fontsize=12)
            plt.tight_layout()
            plt.savefig(fname, dpi=150, bbox_inches='tight')
            plt.close()
...
if RUN_SHAP:
    ...
    for stream_name in INSECTS_STREAMS:
        ...
        if y is None:
            print(f"  y labels not found — skipping SHAP for {stream_name}")
            continue

        for version in ABFS_VERSIONS:
            fname = os.path.join(FIGURES_DIR, f'shap_{version}_{stream_name}.png')
            ...
            for chunk_idx in range(100000):
                ...
                if version == 'aggstats':
                    mf_list.append(extract_metafeatures(wt, wt_prev, drift_count, t_since))
                elif version == 'raw':
                    mf_list.append(extract_metafeatures_raw(wt))
                else:
                    mf_list.append(extract_metafeatures_raw_temporal(wt, wt_prev))
                wt_prev = wt

            X = np.array(mf_list, dtype=float)
            X[np.isnan(X)] = 0; X[np.isinf(X)] = 0

            if version == 'aggstats':
                from metafeatures.mf_extraction import MF_NAMES_AGGSTATS
                feat_names = MF_NAMES_AGGSTATS
            elif version == 'raw':
                feat_names = MF_NAMES_RAW
            else:
                feat_names = MF_NAMES_RAW + ['delta_mean', 'cosine_sim']
            ...
            explainer   = shap.KernelExplainer(mlp.predict_proba, shap.sample(X, 50))
            ...
            fig, ax = plt.subplots(figsize=(max(10, len(feat_names) * 0.6), 4))
            ...
            ax.set_title(f'SHAP feature importance — {ABFS_LABELS[version]}\n'
                         f'{stream_name} (MLP)')
            fig.tight_layout()
            plt.savefig(fname, dpi=150, bbox_inches='tight')
            plt.close()
...
# ============================================================
#  3. ADDITIONAL METRICS HEATMAPS (F1, KAPPA)
# ============================================================
if RUN_METRICS:
    print("\n" + "="*60)
    print("3. METRICS HEATMAPS (F1, KAPPA)")
    print("="*60)
    ...
    for stream_name in INSECTS_STREAMS:
        ...
        for metric in ['f1', 'kappa']:
            ...
            fig.suptitle(
                ...
                f'Prequential evaluation | final window value | '

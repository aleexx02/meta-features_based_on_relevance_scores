...
#
# Streams analysed here: SEA/STAGGER/LED (river synthetic, SEQUENTIAL
# concepts, known exact drift). The stream set + per-stream metadata
# come from streams.generate_synthetic_streams.exp3_specs(), so this
# list stays in sync automatically with the evaluate script and the
# generator.
#
# Two differences from analysis_5.py worth knowing:
#
#  1. Single realization for the per-stream plots. The evaluate script
#     produces (n_reps, n_windows, n_clfs) results, but the sanity /
#     PCA / SHAP / stream-analysis plots only need ONE realization.
#     They use the on-disk stream saved by generate_synthetic_streams.py
#     (the MASTER_SEED realization = replication 0), loaded from
#     data/synthetic/streams[/_gt]. The performance / metrics / gap
#     figures, which summarise the saved results, average over the rep
#     axis (mean over axis 0) before plotting.
#
#  2. Stream-analysis diagnostics are computed INLINE here, not loaded.
#     generate_real_streams.py pre-saves per-chunk diagnostics for the
#     real streams; generate_synthetic_streams.py does not (it only
#     saves the stream + concept_per_chunk), so --stream_analysis
#     recomputes drift intensity / class distribution / label entropy
#     directly from the loaded stream. Same plots, just computed on the
#     fly.
#
# Concept label here = the GENERATIVE concept id per chunk
# (concept_per_chunk on disk), NOT positional. A genuinely distinct
# concept identity, exact by construction.
#
# Figures (results/experiment_3/figures/analysis/):
# --sanity
#   relevance_scores_{stream}.png         ABFS relevance over time + drift lines
#   metafeatures_{version}_{stream}.png   per-version meta-feature trajectories
#   pca_{version}_{stream}.png            2D PCA coloured by concept
#     (feature counts here are small -- 3 for SEA/STAGGER, 24 for LED --
#      so NO feature capping is needed, unlike SPAM in analysis_5.)
# --performance
#   trajectory_abfs_{stream}.png          3 ABFS versions stacked, mean over reps
#   trajectory_komor_{stream}.png         9 Komorniczak groups (3x3), mean over reps
# --shap
#   shap_all_clfs_{version}_{stream}.png  2x2 (GNB/KNN/HT/MLP) per version
# --metrics
#   heatmap_f1_{stream}.png / heatmap_kappa_{stream}.png   mean over reps
# --stream_analysis
#   stream_drift_entropy_{stream}.png     drift intensity / ABFS relevance change /
#                                         label entropy (computed inline)
#   class_distribution_{stream}.png       per-chunk class distribution of the
#                                         real target over time
# --gap
#   gap_heatmap_preq_exp3_{stream}.png    ABFS raw v2.0 minus Komorniczak best-of-9,
#                                         final window, mean over reps, one PER STREAM
#
# Usage:
#   python experiments/experiment_3/analysis_3.py --sanity --performance --shap --metrics --stream_analysis --gap
# ============================================================
...
from streams.generate_synthetic_streams import exp3_specs, CHUNK_SIZE
...
EXP_TAG = 'exp3'
print(f"\nRunning analysis for Experiment 3 (SEA/STAGGER/LED synthetic streams):")
...
def load(prefix, stream_name, optional=False):
    """Load a result .npy from results/experiment_3/. These are

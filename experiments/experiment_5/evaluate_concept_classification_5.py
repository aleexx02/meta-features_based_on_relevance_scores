...
#
# I evaluate all three ReMF meta-feature versions and all 9 Komorniczak
# measure groups on real-world streams with known (or approximately known)
# drift change-point locations. Everything runs in a single script — ReMF
...
#   The real classification target ReMF and Komorniczak both operate on —
...
#   The ground truth concept label for each window — NOT produced by ReMF,
...
#   boundaries passed by this chunk. ReMF plays no role in this label;
...
# Output: ReMF + Komorniczak classifier-sweep results and heatmaps -> results/experiment_5/
...
#   heatmap_comparison_komorniczak_ReMF_preq_exp5_{stream}.png
#     side-by-side: 9 Komorniczak groups vs 3 ReMF versions, final BA
#
...
def already_done_abfs(stream_name, version):
    """Check whether prequential results for this ReMF version already exist."""
...
# ============================================================
#  COMBINED HEATMAP — Komorniczak vs ReMF at final window
# ============================================================
...
def plot_combined_heatmap(stream_name, n_concepts, tba_versions):
    ...
    Komorniczak measure groups (left) and all ReMF versions (right).
    Saved as: heatmap_comparison_komorniczak_ReMF_preq_exp5_{stream}.png
    """
    ...
    fig.suptitle(
        f'Komorniczak vs ReMF — {stream_name}\n'
    ...
    out_path = os.path.join(
        FIGURES_DIR,
        f'heatmap_comparison_komorniczak_ReMF_preq_exp5_{stream_name}.png')
...
# ============================================================
#  ReMF EXTRACTION — all 3 versions in one pass over the stream
# ============================================================
 
def extract_abfs_metafeatures(stream_name, drift_chunks):
    """
    Run ReMF on the stream and extract all three meta-feature versions
    ...
    label that ReMF actually trains its relevance scores against).
...
for stream_name in REAL_STREAMS:
    ...
    if not versions_needed:
        ...
        y_abfs = np.load(os.path.join(RESULTS_DIR,
                                      f'concept_labels_{stream_name}.npy'))
        print_label_dist('ReMF (loaded)', y_abfs)
        for version in ABFS_VERSIONS:
            tba_versions[version] = np.load(
                os.path.join(RESULTS_DIR,
                             f'preq_abfs_{version}_ba_{stream_name}.npy'))
    else:
        print(f"\n  Extracting ReMF meta-features (all 3 versions, one pass)...")
        ...
        for version in ABFS_VERSIONS:
            if already_done_abfs(stream_name, version):
                print(f"  ReMF [{version}] done — loading.")
                tba_versions[version] = np.load(
                    os.path.join(RESULTS_DIR,
                                 f'preq_abfs_{version}_ba_{stream_name}.npy'))
                continue
            print(f"\n  Running prequential (ReMF {version})...")
            ...
            print(f"  [Preq ReMF {version}] " + "  ".join(
                f"{n}={tba[-1, i]:.3f}"
    ...
    for measure in MEASURES:
        ...
        y_abfs_dist  = print_label_dist('ReMF', y_abfs)

...
sys.path.append('..')     # experiments/ → classifier_sweep_komor
sys.path.append('../..') # project root  → plot_results
...
MEASURES = [
    'clustering', 'complexity', 'concept', 'general',
...
for drift_type, n_drifts, concept_sigmoid_spacing, n_concepts in DRIFT_CONFIGS:
    for mf_type, mf_label, mf_names, _ in MF_CONFIGS:
        ...
        ax.set_title(
            f'Performance variance — {mf_label} — {drift_type} drift')
        ax.set_xticks(x + width * 2)
        ax.set_xticklabels(
            [f'Rep {i+1}\n(seed={RANDOM_STATES[i]})' for i in range(N_REPLICATIONS)],
            fontsize=8)
        ax.legend(fontsize=9)
        fig.tight_layout()
        fname = os.path.join(FIGURES_DIR,
                             f'variance_{mf_type}_{drift_type}.png')
        fig.savefig(fname, dpi=150)
        plt.close()
        print(f"Saved: {fname}")
...
for drift_type, n_drifts, concept_sigmoid_spacing, n_concepts in DRIFT_CONFIGS:
    for mf_type, mf_label, mf_names, _ in MF_CONFIGS:
        ...
        ax.set_xticklabels(
            [mf_names[i] for i in sorted_idx],
            rotation=45, ha='right', fontsize=9)
        ax.set_ylabel('Mean absolute SHAP value')
        ax.set_title(
            f'SHAP feature importance — {mf_label} — {drift_type} drift\n'
...
for drift_type, n_drifts, concept_sigmoid_spacing, n_concepts in DRIFT_CONFIGS:
    for metric, metric_label in [('f1', 'Macro F1'), ('kappa', "Cohen's Kappa")]:

        # load Komorniczak baseline (statistical measure group)
        rc_path = os.path.join(RESULTS_DIR,
                               f'clf_replication_{metric}_{drift_type}.npy')
        ...
        ax.set_title(
            f'{metric_label} — {drift_type} drift ({n_concepts} concepts)',
            fontsize=10)
